...
Deno.serve(async (req) => {
  ...
  try {
    const { registration_id, email, pdf_base64 } = await req.json();
    ...
    if (String(data.email ?? "").trim().toLowerCase() !== registrantEmail) {
      return corsJson({ success: false, msg: "Email does not match registration" }, 403);
    }
